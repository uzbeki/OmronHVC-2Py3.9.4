...
# Rectangle position steadiness parameter [0 to 100]
steadiness_param_pos = 30
# Rectangle size steadiness parameter     [0 to 100]
steadiness_param_size = 30
...
def _parse_arg(argv):
    ...
    if arg_count == 3 or arg_count == 4:
        ...
        if baudrate not in p2def.AVAILABLE_BAUD:
            print("Error: Invalid baudrate. Exiting...")
            sys.exit()
        # Gets STB flag
        use_stb = p2def.USE_STB_ON  # Default setting is ON
        if arg_count == 4 and argv[3] == "OFF":
...
def _set_hvc_p2_parameters(hvc_p2_api):
    ...
    # Sets threshold
    res_code = hvc_p2_api.set_threshold(
...
def _set_stb_parameters(hvc_p2_api):
    ...
    if ret != 0:
        raise ValueError(
            "Error: Invalid parameter. set_stb_tr_steadiness_param().")
    ...
    if ret != 0:
        raise ValueError(
            "Error: Invalid parameter. set_stb_pe_threshold_use().")
    ...
    if ret != 0:
        raise ValueError(
            "Error: Invalid parameter. set_stb_pe_complete_frame_count().")
    ...
    if ret != 0:
        raise ValueError(
            "Error: Invalid parameter. set_stb_fr_threshold_use().")
    ...
    if ret != 0:
        raise ValueError(
            "Error: Invalid parameter. set_stb_fr_complete_frame_count().")
...
def main():
    ...
    # Changing to the specified baudrate
    ...
    try:
        ...
        while True:
            ...
            with open('Logs.log', 'a') as logs:
                ...
                logs.write(str(hvc_tracking_result))
            system('cls')
